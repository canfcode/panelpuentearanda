  ...
  $.ajax({
    ...
    success: function(response){
      ...
      if(response == 200){
        ...
        $(event.target).parent().children(".invalid-feedback").html("el nombre ya existe");

      }else{

        validateJS(event, type);

        if(table == "categories" || table == "categoriatramites" || table == "stores" || table == "tramites"){
...
/*$(".summernote").summernote({
  ...
  toolbar:[
      ...
      ['style', ['bold', 'italic', 'underline', 'clear']],
      ['para', ['style', 'ul', 'ol', 'paragraph', 'height']],
      ['insert', ['link', 'picture', 'video', 'hr', 'social']]
  ],
  buttons: {
    social: function (context) {
      var ui = $.summernote.ui;
      var button = ui.button({
        contents: '<i class="fa fa-facebook"></i> Facebook',
        tooltip: 'Insert Facebook Post',
        click: function () {
          var url = prompt('Enter Facebook post URL:');
          if (url) {
            var embedCode = '<div class="fb-post" data-href="' + url + '"></div>';
            context.invoke('editor.insertNode', $(embedCode)[0]);
          }
        }
      });
    ...
    social: function (context) {
      ...
      var button = ui.button({
        ...
        click: function () {
          var url = prompt('Enter Facebook post URL:');
          if (url) {
            var postCode = '<div class="fb-post" data-href="' + url + '"></div>';
            context.invoke('editor.insertNode', $(postCode)[0]);
          }
        }
      });
      return button.render();
