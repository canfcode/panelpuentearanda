	...
	public function login(){
		...
		if(isset($_POST["loginEmail"])){
			...
			if(preg_match('/^[.a-zA-Z0-9_]+([.][.a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["loginEmail"] )){
				...
				$url = "usuarios?login=true&suffix=user";
				$method = "POST";
				$fields = array(
	...
	public function create(){
		...
		if(isset($_POST["displayname"])){
			...
			if(preg_match('/^[A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["displayname"] ) && 
			   ...
			   preg_match('/^[A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["cargo"] ) &&  
			   ...
			   preg_match('/^[#\\=\\$\\;\\*\\_\\?\\¿\\!\\¡\\:\\.\\,\\0-9a-zA-Z]{1,}$/', $_POST["password"] )  ){
				...
				$data = array(
					...
					"rol_user" => "admin",
					"nombre_user" => trim(TemplateController::capitalize($_POST["displayname"])),
					"cargo_user" => trim(TemplateController::capitalize($_POST["cargo"])),
					"usuario_user" => trim(strtolower($_POST["username"])),
				...
				$url = "usuarios?register=true&suffix=user";
				$method = "POST";
				$fields = $data;
				...
				if($response->status == 200){
					...
					if(isset($_FILES["picture"]["tmp_name"]) && !empty($_FILES["picture"]["tmp_name"])){	
						...
						$url = "usuarios?id=".$id."&nameId=id_user&token=".$_SESSION["admin"]->token_user."&table=usuarios&suffix=user";

						$method = "PUT";
						$fields = 'foto_user='.$picture;
						...
						if($response->status == 200){
							...
							echo '<script>
								...
								matPreloader("off");
								fncSweetAlert("close", "", "");
								fncSweetAlert("success", "Usuario creado correctamente", "/admins");
	...
	public function edit($id){
		...
		if(isset($_POST["idAdmin"])){
			...
			echo '<script>
				...
				matPreloader("on");
				fncSweetAlert("loading", "Loading...", "dat");

			</script>';

			if($id == $_POST["idAdmin"]){

				$select = "password_user,foto_user";

				$url = "usuarios?select=".$select."&linkTo=id_user&equalTo=".$id;
				$method = "GET";
				$fields = array();
					...
					if(preg_match('/^[A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["displayname"] ) && 
					   ...
					   preg_match('/^[.a-zA-Z0-9_]+([.][.a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["email"] )  ){
						...
						if(isset($_FILES["picture"]["tmp_name"]) && !empty($_FILES["picture"]["tmp_name"])){	
							...
							$picture = $response->results[0]->foto_user;
						...
						$data = "usuario_user=".trim(TemplateController::capitalize($_POST["displayname"]))."&nombre_user=". trim(strtolower($_POST["username"]))."&email_user=".trim(strtolower($_POST["email"]))."&password_user=".$password."
						&foto_user=".$picture;
						...
						if($response->status == 200){		
							...
							echo '<script>
								...
								matPreloader("off");
								fncSweetAlert("close", "", "");
								fncSweetAlert("success", "Datos actualizados correctamente", "/admins");
