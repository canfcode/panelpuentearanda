...
class CursosController{
	...
	public function create(){
		...
		if(isset($_POST["nombre-dependencia"])){
			...
			=============================================*/		
			if(preg_match('/^[0-9A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,100}$/', $_POST["nombre-dependenia"] ) && 
			preg_match('/^[A-z0-9\\._-]+@[A-z0-9][A-z0-9-]*(\\.[A-z0-9_-]+)*\\.([A-z]{2,6})$/', $_POST["email-dependenia"])
			){
				...
				$data = array(
					"id_dependencia" => $_POST["nombre-dependencia"],
					"correo_negocio" => trim(strtolower($_POST["email-dependencia"])),					
					"fecha_creacion_dependencia" => date("Y-m-d")
				...
				$url = "dependencias?token=".$_SESSION["admin"]->token_user."&table=usuarios&suffix=user";
				$method = "POST";
				$fields = $data;
				...
				if($response->status == 200){
						...
						echo '<script>
							...
							matPreloader("off");
							fncSweetAlert("close", "", "");
							fncSweetAlert("success", "dependencia creado correctamente", "/dependencias");
					...
					echo '<script>
						...
						matPreloader("off");
						fncSweetAlert("close", "", "");
						fncNotie(3, "Error guardando la dependencia");
			...
			if($id == $_POST["idCurso"]){
				...
				if($response->status == 200){
					...
					if(preg_match('/^[0-9A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,100}$/', $_POST["nombre-noticia"] ) && 
						...
						$data ="id_dependencia=".$_POST["nombre-dependencia"].
